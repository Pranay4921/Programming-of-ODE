...
function Huen_Method(f, U_0, V_0, Delt, n) {
    u[0] = U_0;
    v[0] = V_0;
    t[0] = 0;
    temp_u = 0;
    temp_v = 0;
    for (i = 1; i <= n; i++) {
        temp_u = u[i - 1] + Delt * f((i - 1) * Delt, u[i - 1],v[i-1]);
        temp_v = v[i - 1] + Delt * g((i - 1) * Delt, u[i - 1],v[i-1]);
        u[i] = u[i - 1] + 0.5 * Delt * f((i - 1) * Delt, u[i - 1], v[i - 1]) + 0.5 * Delt * f(i * Delt, temp_u, temp_v);
        v[i] = v[i - 1] + 0.5 * Delt * g((i - 1) * Delt, u[i - 1], v[i - 1]) + 0.5 * Delt * g(i * Delt, temp_u, temp_v);
        t[i] = t[i - 1] + Delt;
    }
...
function draw() {
    ...
    stroke(0);
    text('Huen Method (u)', width - width / lpx + 2 * lplw, height / lpy + ld);
    stroke('blue');
    line(width - width / lpx - lplw, height / lpy + 2*ld, width - width / lpx + lplw, height / lpy + 2*ld);
    stroke(0);
    text('Huen Method (v)', width - width / lpx + 2 * lplw, height / lpy + 2*ld);
}