...
function Heun_method(f, U_0, Delt, n) {
    u[0] = U_0;
    t[0] = 0;
    temp_u = 0;
    for (i = 1; i <= n; i++) {
        temp_u = u[i - 1] + Delt * f((i-1) * Delt, u[i - 1]);
        u[i] = u[i - 1] + 0.5 * Delt * f((i - 1) * Delt, u[i - 1]) + 0.5 * Delt * f(i * Delt, temp_u);
        t[i] = t[i - 1] + Delt;
    }
...
function draw() {
    ...
    stroke(0);
    text('Huen Method', width - width / lpx + 2 * lplw, height / lpy+ld);
}