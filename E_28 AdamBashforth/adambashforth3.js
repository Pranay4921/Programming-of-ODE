...
function adambash(f, U_0, Delt, n) {
    u_ab[0] = U_0;
    u_ab[1] = u_ab[0]+Delt*f(0,u_ab[0]);
	u_ab[2] = u_ab[1]+Delt*f(Delt,u_ab[1]);
    for (i = 3; i <= n; i++) {
        u_ab[i] = u_ab[i - 1] + (1/12)*Delt * (23*f((i-1) * Delt, u_ab[i - 1])-16*f((i-2) * Delt, u_ab[i - 2])+5*f((i-3) * Delt, u_ab[i - 3]));
    }
...
function RK4_func(f, U_0, Delt, n) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        K3 = f((i - 0.5) * Delt, u[i - 1] + 0.5 * Delt * K2);
        K4 = f((i) * Delt, u[i - 1] +  Delt * K3);
        u[i] = u[i - 1] + (K1+2*K2+2*K3+K4)*Delt/6;
        t[i] = t[i - 1] + Delt;
    }
...
function draw() {
	...
	//---------------------Numerical Solution AdamBashforth--------------------
    for (i = 1; i <= N; i++) {
        X1 = (t[i-1] + 1) * width / psx;
        Y1 = height - height / psy - u_ab[i - 1] * height / psy;
        X2 = (t[i] + 1) * width / psx;
        Y2 = height - height / psy - u_ab[i] * height / psy;
        stroke('blue');
        strokeWeight(1);
        line(X1, Y1, X2, Y2);
        strokeWeight(2);
    ...
    stroke(0);
    text('RK4', width - width / lpx + 2 * lplw, height / lpy+ld);
		//Leapfrog Legend
	stroke('blue');
    line(width - width / lpx - lplw, height / lpy + 2*ld, width - width / lpx + lplw, height / lpy + 2*ld);
    stroke(0);
    text('AdamBashforth', width - width / lpx + 2 * lplw, height / lpy+2*ld);
