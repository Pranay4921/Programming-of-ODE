...
U0 = Math.exp(-16);                            //Initial Condition                                (CAN BE TUNED)
...
N_exact = 100;                       //Number of points for exact equation               (CAN BE TUNED)
...
function mpitr_func(f, U_0, Delt, n,n_itr) {
    u[0] = U_0;
    t[0] = 0;
    for (i = 1; i <= n; i++) {
        temp1 = u[i-1];
		for(k=1;k<=n_itr;k++){
			temp2 = u[i-1]+0.5*Delt*(f(i*Delt,temp1)+f((i-1)*Delt,u[i-1]));
			temp1 = temp2;
		}
        u[i] = temp2;
        t[i] = t[i - 1] + Delt;
...
function exact_fun(Tcap,n) {
    for (i = 0; i <= n; i++) {
        u_exact[i] = Math.exp(-Math.pow(((i * Tcap / n)-4),2));
        t_exact[i] = i * Tcap / n;
    }
...
function draw() {
    ...
    //---------------------Numerical Solution Mid Point Iteration--------------------
    for (i = 1; i <= N; i++) {
    ...
    text('Exact', width - width / lpx + 2 * lplw, height / lpy);
        //Midpoint Iteration legend
    stroke('red');
    line(width - width / lpx - lplw, height / lpy + ld, width - width / lpx + lplw, height / lpy + ld);
    stroke(0);
    text('Midpoint Iter', width - width / lpx + 2 * lplw, height / lpy+ld);
		//FForward Euler Method
    stroke('blue');
    line(width - width / lpx - lplw, height / lpy + 2*ld, width - width / lpx + lplw, height / lpy + 2*ld);
    stroke(0);
    text('Forward Euler', width - width / lpx + 2 * lplw, height / lpy+2*ld);
