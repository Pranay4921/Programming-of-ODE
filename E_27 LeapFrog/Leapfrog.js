...
function Leapfrog(f, U_0, Delt, n) {
    u_lfrog[0] = U_0;
    u_lfrog[1] = u_lfrog[0]+Delt*f(0,u_lfrog[0]);
    for (i = 2; i <= n; i++) {
        u_lfrog[i] = u_lfrog[i - 2] + 2*Delt * f((i-1) * Delt, u_lfrog[i - 1]);
    }
...
function draw() {
    ...
    for (i = 1; i <= N; i++) {
        X1 = (t[i-1] + 1) * width / psx;
        Y1 = height - height / psy - u_lfrog[i - 1] * height / psy;
        X2 = (t[i] + 1) * width / psx;
        Y2 = height - height / psy - u_lfrog[i] * height / psy;
        stroke('blue');
        strokeWeight(1);
        line(X1, Y1, X2, Y2);
        strokeWeight(2);
    ...
    stroke(0);
    text('Leapfrog', width - width / lpx + 2 * lplw, height / lpy+2*ld);
